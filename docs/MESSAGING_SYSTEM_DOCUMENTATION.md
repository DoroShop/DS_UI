...
```javascript
// Recommended: use `socketManager` which wraps `socket.io-client` and ensures the token is sent in the handshake
import socketManager from 'src/utils/socket';
import { useAuthStore } from 'src/stores/authStores';

// Example usage in a component or setup file
const authStore = useAuthStore();
// Ensure session is fetched and token is available
await authStore.fetchSession();
if (authStore.token) {
  // Connect and let socketManager handle events / reconnection
  const socket = socketManager.connect(authStore.token);
  socketManager.on('connect', () => console.log('✅ Socket connected'));
  socketManager.on('connect_error', (err) => console.error('🔴 Socket connection error', err));
} else {
  console.warn('No auth token found — socket connections will be rejected by the server');
}
