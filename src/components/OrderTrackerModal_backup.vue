...
let _onNewAgreementMessage = null;
...
const connectSocket = () => {
  ...
  if (!authStore.token) authStore.fetchSession().catch(() => {});
  const token = authStore.token;
  if (!token) {
    console.warn('[SOCKET] No auth token; socket will not connect until authenticated');
    return;
  }
...
onUnmounted(() => {
  if (props.orderId) socketManager.emit('leave_order', props.orderId);
